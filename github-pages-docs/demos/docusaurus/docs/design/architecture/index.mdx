...
import OrderStateStepper from '@site/src/components/OrderStateStepper';
...
![Acme Order Service system architecture](/img/architecture.svg)
...
`validated`. The interactive stepper below walks through the states; the
sequence diagram below it shows the happy path for placing and paying an
order.
...
![Acme Order Service deployment pipeline](/img/deploy-flow.svg)
