...
![Acme Order Service system architecture](/architecture.svg)
...
### Interactive: order state stepper

The stepper below is a Vue single-file component embedded directly in this
markdown page (VitePress compiles Vue components inside `.md` files — no
MDX plugin required). It mirrors the lifecycle above, including the
`cancelled` branch reachable from `created` or `validated`.
...
![Acme Order Service deployment pipeline](/deploy-flow.svg)
